...
+ (BOOL)unzipFileAtPath:(NSString *)path
    ...
    do {
        @autoreleasepool {
            ...
            if (nil != err) {
                SMLog(@"ZipArchive Error: %@", err.localizedDescription);
            }
            ...
            if (!fileIsSymbolicLink) {
                ...
                if (fp) {
                    if ([[[fullPath pathExtension] lowercaseString] isEqualToString:@"zip"]) {
                        SMLog(@"Unzipping nested .zip file:  %@", [fullPath lastPathComponent]);
                    ...
                    if (0 != fileInfo.dosDate) {
                        ...
                        if (attributes) {
                            if (NO == [fileManager setAttributes:attributes
                                ...
                                SMLog(@"[ZipArchive] Failed to set attributes - whilst setting modification date");
                            }
                        }
                    }
                    ...
                    if (0 != permissions) {
                        ...
                        if (NO == [fileManager setAttributes:attributes
                            ...
                            SMLog(@"[ZipArchive] Failed to set attributes - whilst setting permissions");
                        }
                    }
                }
            } else {
                ...
                if(symlinkError != 0) {
                    SMLog(@"Failed to create symbolic link at \"%@\" to \"%@\". symlink() error code: %d", fullPath, destinationPath, errno);
                }
            }
    ...
    for (NSDictionary *dictionary in directoriesModificationDates) {
        ...
        if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: dictionary[@"modDate"]}
                                              ofItemAtPath:dictionary[@"path"]
                                                     error:&err]) {
            SMLog(@"[ZipArchive] Set attributes failed for directory: %@.", dictionary[@"path"]);
        }
        
        if (err) {
            SMLog(@"[ZipArchive] Error setting directory file modification date attribute: %@", err.localizedDescription);
        }
    }
